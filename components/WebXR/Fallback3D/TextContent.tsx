...
const AnimatedText = React.memo<AnimatedTextProps>(
  ({ text, position, size, color, delay, animationType = 'fade', quality }) => {
    ...
    useFrame((state) => {
      ...
      switch (animationType) {
        case 'fade':
          if (material instanceof THREE.Material) {
            material.opacity =
              ANIMATION_CONFIG.fade.base +
              Math.sin(time * ANIMATION_CONFIG.fade.frequency) *
                ANIMATION_CONFIG.fade.amplitude
          }
          break
        case 'scale':
          textRef.current.scale.setScalar(
            ANIMATION_CONFIG.scale.base +
              Math.sin(time * ANIMATION_CONFIG.scale.frequency) *
                ANIMATION_CONFIG.scale.amplitude,
          )
          break
        case 'slide':
          textRef.current.position.y =
            position[1] +
            Math.sin(time * ANIMATION_CONFIG.slide.frequency) *
              ANIMATION_CONFIG.slide.amplitude
          break
      }
      ...
      if (material instanceof THREE.MeshStandardMaterial) {
        material.emissiveIntensity =
          ANIMATION_CONFIG.glow.base +
          Math.sin(time * ANIMATION_CONFIG.glow.frequency) *
            ANIMATION_CONFIG.glow.amplitude
      }
    })

    const materialProps = useMemo(
      () => createStandardMaterial(color, 0.2, true),
      [color],
    )
...
const Text3D = React.memo<Text3DProps>(
  ({
    ...
    const materialProps = useMemo(
      () => createStandardMaterial(color, emissiveIntensity),
    ...
    const textProps = useMemo(
      () => ({
        ...
        onError: (error: any) => {
          console.warn(
            'Font loading error, falling back to system font:',
            error,
          )
        },
...
const FloatingTextBubble = React.memo<{
  ...
  const sphereGeometry = useMemo(
    () =>
      [text.length * 0.15, qualityConfig.segments, qualityConfig.segments] as [
        number,
        number,
        number,
      ],
...
const HERO_TEXT_LINES = [
  ...
  {
    text: 'or building a',
    position: [-1.5, -2.5, 0] as [number, number, number],
    size: 1.0,
    color: '#9ca3af',
    delay: 1.8,
    emissiveIntensity: 0.2,
...
const TextContent = React.memo<TextContentProps>(({ quality }) => {
  ...
  return (
    <group position={[0, 0, -10]}>
      ...
      {showHelpText && (
        <Html position={[0, -4, 0]} transform occlude style={HELP_TEXT_STYLE}>
          <div className="animate-pulse">↑ Scroll or drag to explore ↑</div>
        </Html>
      )}
    </group>
  )
