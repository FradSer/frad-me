...
jest.mock('@react-three/drei', () => ({
  Text: ({ children, color, position, font, anchorX, anchorY, fontSize, rotation, ...props }: any) => (
...
describe('HeroText Component', () => {
  ...
  describe('View-based Animations', () => {
    it('starts with home animation state', () => {
      const { result } = renderHook(
        () => ({
          positionX: useSimpleLerp(heroAnimationStates.home.position.x),
          positionY: useSimpleLerp(heroAnimationStates.home.position.y),
          positionZ: useSimpleLerp(heroAnimationStates.home.position.z),
          opacity: useSimpleLerp(heroAnimationStates.home.opacity),
        }),
        {
          wrapper: ({ children }) => (
            <WebXRViewProvider initialView="home">
              {children}
            </WebXRViewProvider>
          ),
        }
    ...
    it('transitions to hidden state when view changes to work', () => {
      const { result } = renderHook(
        () => {
          ...
          const positionY = useSimpleLerp(heroAnimationStates.home.position.y);
          const opacity = useSimpleLerp(heroAnimationStates.home.opacity);
    ...
    it('handles visibility based on opacity threshold', () => {
      ...
      groupRef.current = mockGroup as any;
  ...
  describe('Interactive Mesh Hook', () => {
    ...
    it('maintains active state on hover out', () => {
      const { result } = renderHook(
        () => {
          const [hovered, setHovered] = React.useState(false);
          const [active, setActive] = React.useState(true); // Start active
  ...
  describe('Text Rendering', () => {
    it('renders text with correct properties', () => {
      ...
      expect(heroContent.some(item => item.text?.includes('is a self-taught craftier'))).toBe(true);
      expect(heroContent.some(item => item.text?.includes('who is eager to learn for'))).toBe(true);
    });
  ...
  describe('Animation Performance', () => {
    ...
    it('applies material opacity to all child materials', () => {
      ...
      if (Array.isArray(mockMesh.material)) {
        mockMesh.material.forEach((material: any) => {
  ...
  describe('Content Configuration', () => {
    ...
    it('handles different content types in renderElement', () => {
      const elements = [
        { type: 'line', position: [0, 0, 0] as [number, number, number], color: 'white', text: 'Test' },
        { type: 'box', position: [1, 1, 1] as [number, number, number] },
        { type: 'triangle', position: [2, 2, 2] as [number, number, number], rotation: [0, 0, 0] as [number, number, number], scale: 1 },
