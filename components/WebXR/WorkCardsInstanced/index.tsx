...
const WorkCardsInstanced = memo<WorkCardsInstancedProps>(function WorkCardsInstanced({
  ...
  return (
    <group aria-label="Portfolio work cards">
      <ambientLight intensity={0.4} />
      <directionalLight position={[5, 5, 5]} intensity={0.8} />
      <pointLight position={[-5, 3, 2]} intensity={0.6} color="#60a5fa" />

      <Suspense fallback={null}>
        {/* biome-ignore lint/a11y/noStaticElementInteractions: Three.js instancedMesh cannot be replaced with button element */}
        <instancedMesh
          ref={meshRef}
          ...
          onClick={handleClick}
          userData={{
            'aria-label': displayWorks.map((work) => `${work.title} - ${work.subTitle}`),
          }}
        >
