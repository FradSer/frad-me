...
export type AnimationType = 'float' | 'rotate' | 'scale' | 'orbit' | 'pulse' | 'bounce' | 'sway'
...
const DEFAULT_CONFIGS: Record<AnimationType, Partial<AnimationConfig>> = {
  float: { speed: 1, intensity: 1, amplitude: 0.5, frequency: 1, easing: easingFunctions.sine },
  ...
  orbit: { speed: 0.5, intensity: 1, radius: 2, centerOffset: [0, 0, 0], axis: 'y', easing: easingFunctions.linear },
  pulse: { speed: 2, intensity: 1, property: 'both', min: 0.3, max: 1.0, easing: easingFunctions.sine },
...
const calculateAnimationState = (time: number, config: AnimationConfig) => {
  const { speed = 1, delay = 0, direction = 'forward', easing = easingFunctions.linear } = config
...
const updateMaterialProperty = (
  ...
  const material = object.material as THREE.Material & { [key: string]: unknown }
...
const animationFunctions = {
  float: (ref: React.RefObject<THREE.Object3D>, state: ReturnType<typeof calculateAnimationState>, config: AnimationConfig, initialPosition?: THREE.Vector3) => {
    const { amplitude = 1, frequency = 1, intensity = 1 } = config
    const { adjustedTime, directionMultiplier } = state
    const offset = Math.sin(adjustedTime * frequency) * amplitude * intensity * directionMultiplier
  ...
  rotate: (ref: React.RefObject<THREE.Object3D>, state: ReturnType<typeof calculateAnimationState>, config: AnimationConfig) => {
  ...
  scale: (ref: React.RefObject<THREE.Object3D>, state: ReturnType<typeof calculateAnimationState>, config: AnimationConfig) => {
  ...
  orbit: (ref: React.RefObject<THREE.Object3D>, state: ReturnType<typeof calculateAnimationState>, config: AnimationConfig, initialPosition?: THREE.Vector3) => {
    const { radius = 2, centerOffset = [0, 0, 0], axis = 'y', intensity = 1 } = config
    const { adjustedTime, directionMultiplier } = state
    ...
    const axisMap = {
      y: { x: initialPosition.x + cx + cosAngle, z: initialPosition.z + cz + sinAngle },
      x: { y: initialPosition.y + cy + cosAngle, z: initialPosition.z + cz + sinAngle },
      z: { x: initialPosition.x + cx + cosAngle, y: initialPosition.y + cy + sinAngle }
    }[axis as Axis]
    
    Object.assign(ref.current.position, axisMap)
  },

  pulse: (ref: React.RefObject<THREE.Object3D>, state: ReturnType<typeof calculateAnimationState>, config: AnimationConfig) => {
  ...
  bounce: (ref: React.RefObject<THREE.Object3D>, state: ReturnType<typeof calculateAnimationState>, config: AnimationConfig, initialPosition?: THREE.Vector3) => {
    const { intensity = 1 } = config
    const { adjustedTime, directionMultiplier } = state
    const bounceValue = Math.abs(Math.sin(adjustedTime * 4)) * intensity * directionMultiplier
    
    if (ref.current) {
      updatePositionOnAxis(ref.current.position, initialPosition, 'y', bounceValue)
    }
  },

  sway: (ref: React.RefObject<THREE.Object3D>, state: ReturnType<typeof calculateAnimationState>, config: AnimationConfig, initialPosition?: THREE.Vector3) => {
    const { intensity = 1 } = config
    const { adjustedTime, directionMultiplier } = state
    const swayX = Math.sin(adjustedTime) * intensity * 0.1 * directionMultiplier
    const swayZ = Math.cos(adjustedTime * 0.7) * intensity * 0.1 * directionMultiplier
...
export const useFloatAnimation = createAnimationHook<FloatAnimationConfig>('float', true)
export const useRotationAnimation = createAnimationHook<RotationAnimationConfig>('rotate', false)
export const useScaleAnimation = createAnimationHook<ScaleAnimationConfig>('scale', false)
export const useOrbitAnimation = createAnimationHook<OrbitAnimationConfig>('orbit', true)
export const usePulseAnimation = createAnimationHook<PulseAnimationConfig>('pulse', false)
export const useBounceAnimation = createAnimationHook<AnimationConfig>('bounce', true)
export const useSwayAnimation = createAnimationHook<AnimationConfig>('sway', true)
...
export const useCombinedAnimation = (
  ...
  const needsInitialPosition = useMemo(() => 
    animations.some(({ type }) => ['float', 'orbit', 'bounce', 'sway'].includes(type))
  , [animations])
...
export const useSpringAnimation = (
  ...
  const updateSpring = useCallback((deltaTime: number) => {
    ...
    if (typeof target === 'number') {
      ...
      ;(position.current as number) += (velocity.current as number) * deltaTime
      ...
      tempVectors.acceleration.copy(tempVectors.force).sub(tempVectors.dampingForce).divideScalar(mass)
      
      tempVectors.velocityDelta.copy(tempVectors.acceleration).multiplyScalar(deltaTime)
