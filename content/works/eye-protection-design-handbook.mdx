...
When peoples watch the screen at close range, the eyes perform tonal and introverted movements. The closer the object is, the more pressure is placed on the eye muscles. The eye muscles cannot relax for a long time, thus causing visual fatigue.

[ ] Image Here.

- **Accommodation** is When viewing objects at close distances, the ciliary muscles contract, and the lens lengthens, focusing on objects at close distances.
- **Entropion** is the contraction of the internal rectus muscles of both eyes inward, resulting in seeing a clear image rather than a double image.
...
Nevertheless, keep in mind that when using devices such as cell phones and tablets, they all belong to the eyes working at close range, and the relief of visual fatigue is minimal. If people can use the screencasting function to display the content on the TV and projection, the comfort level for the eyes is ideal. The effect is even better when the mobile device becomes a controller-like device that can be linked to the content on the TV.
...
Take iPhone X as an example, an adult male watching a video in landscape. The three distances are as follows:
...
Comfortable font size for body text should have a stroke thickness equal to or greater than one logical pixel in height. Take PingFang Regular as an example. When its stroke thickness equals 1pt, its font size is 14.5pt. When people sit at eye comfort distance, the 14.5pt appendix looks comfortable and eye-friendly. Keep in mind that this size also varies for different font weights of the same font family or different font families.
...
Of course, it is technically possible to change the color temperature of a layer directly by referring to this part of the code:
