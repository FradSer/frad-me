...
describe('Error Logger Utility', () => {
  ...
  describe('Offline Queue Support', () => {
    ...
    it('should retry queued errors when back online', async () => {
      ...
      await webxrErrorLogger.logError(new Error('Trigger error'), {
        component: 'Trigger',
      });
  ...
  describe('Error Recovery and Monitoring', () => {
    it('should handle API failures gracefully', async () => {
      ...
      await expect(
        webxrErrorLogger.logError(testError, { component: 'APITest' }),
  ...
  describe('Performance Impact', () => {
    ...
    it('should handle high-frequency error logging', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        errors.push(
          webxrErrorLogger.logError(new Error(`High freq error ${i}`), {
            component: 'HighFreq',
            index: i,
          }),
        );
