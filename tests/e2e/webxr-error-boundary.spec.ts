...
class WebXRErrorBoundaryPage extends BasePage {
  ...
  async verifyWebXRMode() {
    ...
    await expect(this.page.getByText('WebXR Experience Unavailable')).not.toBeVisible()
  }

  async verifyErrorBoundaryMessage() {
    await expect(this.page.getByText('WebXR Experience Unavailable')).toBeVisible()
  }

  async verifyErrorButtons() {
    await expect(this.page.locator('button:has-text("Try Again")')).toBeVisible()
    await expect(this.page.locator('button:has-text("Return to Main")')).toBeVisible()
  }
