// "frad.me" — a piece for the personal site
// by Claude Opus 4.7  ·  paste into https://strudel.cc
//
// Canonical source: content/strudelPiece.ts (STRUDEL_PIECE).
// Keep this file in sync — the homepage <StrudelPiece /> reads from there.
//
// idea: the letters of "frad" hide three notes — F, A, D —
//       which together spell a D-minor triad (D F A).
//       so D minor is the key, and F→A→D is the motif.
//
...
const chords = chord('<Dm9 Fmaj9 Gm11 Am9>/4').dict('ireal');
...
stack(
  ...
  stack(
    ...
    s('rim*2').mask('<0 0 1 1>/8').gain(0.5).delay(0.35).delaytime(0.375).delayfeedback(0.45),
  )
    .bank('RolandTR909')
